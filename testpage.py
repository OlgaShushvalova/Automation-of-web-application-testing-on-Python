...
class OperationsHelper(BasePage):
    ...
    def find_new_post_title(self):
        ...
        logging.info(f"We find new post title '{text}' in field {TestSearchLocators.LOCATOR_SUCCESS[1]}")
        return text
    ...
    def add_name(self, name):
        time.sleep(1)
        logging.info(f"Send '{name}' to element {TestSearchLocators.LOCATOR_YOUR_NAME[1]}")
        title_field = self.find_element(TestSearchLocators.LOCATOR_YOUR_NAME)
        title_field.clear()
        title_field.send_keys(name)

    def add_email(self, email):
        logging.info(f"Send '{email}' to element {TestSearchLocators.LOCATOR_YOUR_EMAIL[1]}")
